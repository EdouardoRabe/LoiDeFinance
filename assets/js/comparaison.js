...
function selectedYears() {
  return $('.year-select').map(function() { return $(this).val(); }).get();
}
...
function load() {
  ...
  $.getJSON(url + '?' + params).then(rows => {
    ...
    rows.forEach(r => {
      ...
      if (!byCat[key]) {
        byCat[key] = { nom: r.nom, description: r.description, type: r.type || '', values: {} };
      }
    ...
    Object.values(byCat).forEach(obj => {
      const $row = $('<tr></tr>');
      const $catCell = $(`<td class="category-cell" data-desc="${obj.description || ''}">${obj.nom}</td>`);
      attachTooltipEvents($catCell);
      $row.append($catCell);
      $row.append(`<td>${obj.type}</td>`);
      selectedYears().forEach(y => { $row.append(`<td class="num">${fmt(obj.values[y] || 0)}</td>`); });
      $tb.append($row);
    });
